...
/**
 * https://github.com/byronlai/lock-free-data-structures/blob/master/LinkedList.java
 *
 * @param <T>
 */
public class AtomicLinkedList<T> {
    ...
    public AtomicLinkedList() {
        Node<T> headNode = new Node<>(null, null);
        ...
        while (!successful) {
            ...
            for (Node<T> node = head.getReference(); node != null && !isRemoved(node);
                ...
                if (isEqual(node.value, after) && !node.next.isMarked()) {
                    ...
                    successful = node.next.compareAndSet(nextNode, newNode, false, false);
                    break;
                }
            }
            if (!found) {
        ...
        while (!successful) {
            ...
            for (Node<T> node = head.getReference(), nextNode = node.next.getReference();
                ...
                if (!isRemoved(nextNode) && isEqual(nextNode.value, value)) {
                    ...
                    successful = physicallyRemove(node, nextNode);
                    break;
                }
            }
            if (!found) {
    ...
    private class Node<T> {
        private volatile AtomicMarkableReference<Node<T>> next;
